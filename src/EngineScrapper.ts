...
export class EngineScrapper {
    ...
    async getBrandInfo(brand: String)
    {
        ...
        if(productionModels)
        {
            numberProductionModels = await page.evaluate(element => element.textContent, productionModels);
        }
        else
        {
        ...
        {
            numberDiscountinedModels = await page.evaluate(element => element.textContent, discountinedModels);
        }
        else
        {
